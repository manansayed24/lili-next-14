...
function InterviewForm() {
  const apiKey='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'
  const [file, setFile] = useState(null);
  const [updateData, setUpdateData] = useState([]);
  const [resumeText, setResumeText] = useState('');
  const [keyValues, setKeyValues] = useState({ name: '', email: '', phone: '' });
...
const handleDeselectFile = (fileName) => {
  ...
  const extractKeyValues = (text) => {
    const nameMatch = text.match(/(?:Name|Full Name|Candidate|Applicant):?\s*([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)/i);
  ...
  return (
    ...
    <form className="max-w-lg mx-auto p-8 space-y-6 bg-white bg-opacity-10 shadow-lg rounded-lg" onSubmit={handleSubmit}>
      <div>
