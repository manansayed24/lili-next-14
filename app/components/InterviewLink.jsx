...
const InterviewLink = ({ link }) => {
  ...
  return (
    <div className="flex flex-col items-center p-4 bg-gray-100 bg-opacity-10 rounded-lg shadow-md mt-20">
        <h1>You can follow this link by pressing go button or just copy and paste </h1>
      <a href={`http://localhost:3000/interviewbot?id=${randomId}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 mb-4">
      ...
      {copySuccess && <p className="mt-2 text-sm text-gray-600">{copySuccess}</p>}
    </div>
  );
};
